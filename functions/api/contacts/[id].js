...
export async function onRequest(context) {
  ...
  try {
    if (method === 'GET') {
      return await handleGetSingle(env.DB_CIPTA, tableName, id, corsHeaders);
    } else if (method === 'PUT') {
      return await handleUpdate(request, env.DB_CIPTA, tableName, id, corsHeaders);
    } else if (method === 'DELETE') {
      return await handleDelete(env.DB_CIPTA, tableName, id, corsHeaders);
    } else {
...
async function createTableIfNotExists(DB_CIPTA, tableName) {
  ...
  await DB_CIPTA.prepare(createTableQuery).run();
  console.log(`Table '${tableName}' created or already exists`);
}

// GET single record by id_x - Dynamic table version with auto-create
async function handleGetSingle(DB_CIPTA, tableName, id, corsHeaders) {
  console.log(`--- GET single record from ${tableName} with id_x ${id} ---`);
  
  try {
    // Check if table exists first
    const tableCheck = await DB_CIPTA.prepare(`
      SELECT name FROM sqlite_master WHERE type='table' AND name=?
    `).bind(tableName).first();
    
    if (!tableCheck) {
      console.log(`Table '${tableName}' does not exist, creating...`);
      await createTableIfNotExists(DB_CIPTA, tableName);
    }
    
    const query = `SELECT * FROM ${tableName} WHERE id_x = ?`;
    const result = await DB_CIPTA.prepare(query).bind(id).first();
...
async function handleUpdate(request, DB_CIPTA, tableName, id, corsHeaders) {
  ...
  try {
    ...
    const existsResult = await DB_CIPTA.prepare(existsQuery).bind(id).first();
    ...
    const updateResult = await DB_CIPTA.prepare(updateQuery).bind(...updateValues).run();
...
async function handleDelete(DB_CIPTA, tableName, id, corsHeaders) {
  ...
  try {
    ...
    const deleteResult = await DB_CIPTA.prepare(deleteQuery).bind(id).run();
