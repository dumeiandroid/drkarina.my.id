...
export async function onRequest(context) {
  ...
  try {
    switch (method) {
      case 'GET':
        return await getContacts(DB_KARINA, tableName, corsHeaders);
      case 'POST':
        return await createContact(request, DB_KARINA, tableName, corsHeaders);
      default:
...
async function createTableIfNotExists(DB_KARINA, tableName) {
  ...
  await DB_KARINA.prepare(createTableQuery).run();
  console.log(`Table '${tableName}' created or already exists`);
}

// GET all contacts - Dynamic table version with auto-create
async function getContacts(DB_KARINA, tableName, corsHeaders) {
  console.log(`Getting all data from table: ${tableName}...`);
  
  try {
    // Check if table exists first
    const tableCheck = await DB_KARINA.prepare(`
      SELECT name FROM sqlite_master WHERE type='table' AND name=?
    `).bind(tableName).first();
    
    if (!tableCheck) {
      console.log(`Table '${tableName}' does not exist, creating...`);
      await createTableIfNotExists(DB_KARINA, tableName);
    }
    
    const query = `SELECT * FROM ${tableName} ORDER BY id_x DESC`;
    const { results } = await DB_KARINA.prepare(query).all();
...
async function createContact(request, DB_KARINA, tableName, corsHeaders) {
  ...
  try {
    ...
    const result = await DB_KARINA.prepare(query).bind(...values).run();
